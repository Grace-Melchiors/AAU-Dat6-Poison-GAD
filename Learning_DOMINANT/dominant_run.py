...
def loss_func(adj, A_hat, attrs, X_hat, alpha):
    """
    Calculates the loss of the attribute reconstruction and the Structure reconstruction
    """
    # Attribute reconstruction loss,  pow(input, exponent)
    diff_attribute = torch.pow(X_hat - attrs, 2)
    attribute_reconstruction_errors = torch.sqrt(torch.sum(diff_attribute, 1)) #sum(input, dimension)
    attribute_cost = torch.mean(attribute_reconstruction_errors)

    # Structure reconstruction loww
    diff_structure = torch.pow(A_hat - adj, 2)
    structure_reconstruction_errors = torch.sqrt(torch.sum(diff_structure, 1))
    structure_cost = torch.mean(structure_reconstruction_errors)

    # the alpha value here, determines the weight we put on structure vs. attribute error as the determening factor of a node being a outlier
...
def train_dominant(args):
    ...
    # creates dominant model and passes arguments
    model = Dominant(feat_size = attrs.size(1), hidden_size = args.hidden_dim, dropout = args.dropout)

    # performance related (moves data to gpu if available)
    if args.device == 'cuda':
    ...
    for epochs in range(args.epoch): 
        ...
        # get predicted or reconstructed adjacency matrix and reconstrcuted attributes 
        A_hat, X_hat = model(attrs, adj)

        # calculate loss:  loss, train-structure loss, train-feature loss
        loss, struct_loss, feat_loss = loss_func(adj_label, A_hat, attrs, X_hat, args.alpha)

        # updates parameters in back propagation 
        l = torch.mean(loss)  # average train loss
        l.backward()
        optimizer.step()   
        ...
        if epoch%10 == 0 or epoch == args.epoch - 1: # every 10 epochs and last epoch
            model.eval()
            A_hat, X_hat = model(attrs, adj)
            loss, struct_loss, feat_loss = loss_func(adj_label, A_hat, attrs, X_hat, args.alpha)
