...
from gad_adversarial_robustness.gad.RTGNN.model.Dual_GCN import GCN, Dual_GCN
...
class RTGNN(nn.Module):
    def __init__(self, args, device):
        ...
        self.best_acc_pred_val = 0  #best accuracy prediction validation?
        self.best_pred = None # best prediction?
        self.best_graph = None # best graph?
        self.weights = None
        self.estimator = None
        self.model = None
        self.pred_edge_index = None # predicted edge index?
    ...
    def fit(self, features, adj, labels, true_labels, idx_train, idx_val, noise_idx, clean_idx):
        """
        fit the model to the data (i.e. train)
        """
        ...
        # stores the various data components to the model
        self.edge_index = edge_index
        self.features = features
        labels = torch.LongTensor(np.array(labels)).to(self.device) # labels to tensor
        self.labels = labels
        self.true_labels = true_labels
        self.idx_unlabel = torch.LongTensor(list(set(range(features.shape[0])) - set(idx_train))).to(self.device) # index of unlabeled nodes to tensor
        self.idx_train = torch.LongTensor(idx_train).to(self.device)

        # sets the predictor to the Dual_GCN which was IMPORTED FROM "Dual_GCN.py"
