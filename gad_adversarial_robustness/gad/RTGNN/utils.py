...
def build_pair_p(size, noise):
    # Assert that noise is within the range [0, 1], otherwise raise Assertion Error
    assert(noise >= 0.) and (noise <= 1.)

    # np.eye() Creates a square matrix of size 'size' with all zeros except the diagonal
    # Multiply it by (1.0 - noise), which scales the diagonal elements
    P = (1.0 - np.float64(noise)) * np.eye(size)

    # loop through each row of the matrix
    for i in range(size):
        # add noise to the element at index (i, i-1) in the matrix
        P[i,i-1] = np.float64(noise)
        
    # Assert that the sum of each row of the matrix is approximately equal to 1
    # This assertion ensures that each row of the matrix sums up to 1 (a valid probability distribution)
    assert_array_almost_equal(P.sum(axis=1), 1, 1)
    return P
...
def noisify_with_P(y_train, train_num,  nb_classes, noise, random_state=None,  noise_type='uniform'):
    """
    Adds noise to the labels of y_train set 

    returns:   The noisy y_train set, noise matrix P, indicies of the noisy and clean labels
    """
    if noise > 0.0:
