        ...
        for epoch in range(config["model"]["epochs"]):
            ...
            if verbose:
                print(
                    ...
                    f"train/ddpm_loss={ddpm_loss.item():.5f}"
                )

            self.writer.add_scalar("Loss/train", loss.item(), epoch)  # Log training loss
            self.writer.add_scalar("Loss/structure", struct_loss.item(), epoch)  # Log structure loss
            self.writer.add_scalar("Loss/feature", feat_loss.item(), epoch)  # Log feature loss
            self.writer.add_scalar("Loss/ddpm", ddpm_loss.item(), epoch)  # Log DDPM loss
            ...
            if (epoch % 10 == 0 and verbose) or epoch == config["model"]["epochs"] - 1:
                ...
                eval_loss = torch.mean(loss)
                self.writer.add_scalar("Eval/loss", eval_loss.item(), epoch)  # Log evaluation loss
                self.writer.add_scalar("Eval/auc", roc_auc_score(self.label, score), epoch)  # Log AUC score
